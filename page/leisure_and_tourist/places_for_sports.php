...
if(isset($data['all_items']))
{
    ...
    $db = new Database();
    $db->connect();
    //$db->select('places_for_sports','*','','id');//запрос
    $db->select('attraction','*','type=1','id');//запрос
    $item = $db->getResult();
    $db->getResultNull();
    $db->select('services','*','id=7');
    $res = $db->getResult();
...
else
{
    ...
    $db->connect();
    $db->getResultNull();
    //$db->select('places_for_sports','*','','id  limit 3');//запрос
    $db->select('attraction','*','type=1','id  limit 3');//запрос
    $item = $db->getResult();
    //var_dump($item);
    $db->getResultNull();
    $db->select('services','*','id=7');
...
<html lang="ru">
  ...
  <body>
    ...
    <section style="background:url(../../img/leisure_and_tourist/bg-places_for_sports.jpg); background-repeat: no-repeat;  background-size: cover;" class="lat_services">
      <div class="lat_services-wrap"><a href="/page/leisure_and_tourist/places_for_sports.html" class="lat_services-title"><span><?php echo $res[title] ; ?></span></a>
        <div class="lat_services-content">
          ...
          <div class="lat_services-list">
            ...
            <form method="post" action="/page/leisure_and_tourist/places_for_sports.php">
