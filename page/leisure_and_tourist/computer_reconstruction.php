...
if(isset($data['all_items']))
{
    ...
    $db = new Database();
    $db->connect();
    //$db->select('computer_reconstruction','*','','id');//запрос
    $db->select('attraction','*','type=3','id');//запрос
    $item = $db->getResult();
    $db->getResultNull();
    $db->select('services','*','id=5');
    $res = $db->getResult();
...
else
{
    $db = new Database();
    $db->connect();
   // $db->select('computer_reconstruction','*','','id  limit 3');//запрос
    $db->select('attraction','*','type=3','id  limit 3');//запрос
    $item = $db->getResult();
    $db->getResultNull();
    $db->select('services','*','id=5');
    $res = $db->getResult();
...
<html lang="ru">
  ...
  <body>
    ...
    <section style="background:url(../../img/leisure_and_tourist/bg-computer_reconstruction.jpg); background-repeat: no-repeat;  background-size: cover;" class="lat_services">
      <div class="lat_services-wrap"><a href="/page/leisure_and_tourist/computer_reconstruction.html" class="lat_services-title"><span><?php echo $res[title] ; ?></span></a>
        <div class="lat_services-content">
          ...
          <div class="lat_services-list">
            ...
            <form method="post" action="/page/leisure_and_tourist/computer_reconstruction.php">
